...
class SequenceAnomalyModel(Module):

    def __init__(self, input_size, hidden_size, output_size, num_layers=2):
        """Sequence anomaly model used for training and predicting logs.
        ...
        super(SequenceAnomalyModel, self).__init__()
    ...
    def forward(self, X):
        """Forward sample through the sequence anomaly model.

            Parameters
            ----------
            X : tensor
                Input to forward through the sequence anomaly network.
    ...
    def predict_with_anomaly(self, X, y, k=5, variable=False, verbose=False):
        """Predict events and detect anomalies using top-k strategy.

            Parameters
            ----------
            X : torch.Tensor of shape=(n_samples, seq_len)
                Input sequences of indices

            y : torch.Tensor of shape=(n_samples,)
                True events (ground truth)

            k : int, default=5
                Number of top predictions to check against ground truth
            ...
            Returns
            -------
            predictions : torch.Tensor of shape=(n_samples, k)
                k most likely predicted events

            confidence : torch.Tensor of shape=(n_samples, k)
                Confidence levels for predictions

            anomaly_labels : torch.Tensor of shape=(n_samples,)
                Binary labels: 0 = normal (true event in top-k), 1 = anomaly
            """
        # Get top-k predictions with confidence
        predictions, confidence = self.predict(
            X, y=None, k=k, variable=variable, verbose=verbose
        )

        # Detect anomalies: if true event is NOT in top-k predictions, label as anomaly
        y = y.to(predictions.device)
        
        # Check if true event is in the top-k predictions for each sample
        # Compare y (n_samples,) with predictions (n_samples, k)
        is_in_topk = torch.zeros(len(y), dtype=torch.bool, device=predictions.device)
        for i in range(len(y)):
            is_in_topk[i] = (y[i] in predictions[i])

        # Anomaly label: 1 if NOT in top-k, 0 if in top-k
        anomaly_labels = (~is_in_topk).long()

        return predictions, confidence, anomaly_labels
